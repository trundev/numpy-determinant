...
def test_custom_dtype():
    ...
    class my_dtype:
        ...
        def __mul__(self, obj):
            nonlocal mul_cnt
            mul_cnt += 1
            return my_dtype(self.val * obj.val)
        def __neg__(self):
            nonlocal neg_cnt
            neg_cnt += 1
            return my_dtype(-self.val)
        def __add__(self, obj):
            nonlocal add_cnt
            add_cnt += 1
            return my_dtype(self.val + obj.val)
    print(f'Data-type object operations: {set(dir(my_dtype)) - set(dir(object))}')

    # Create array of my_dtype objects and calc its determinant
    data = np.vectorize(my_dtype)(MATRIX_DET_3)
    res = determinant.det(data)
    # Validate the result
    assert isinstance(res, my_dtype), 'Determiant data-type was changed'
    ...
    print(f'Data-type object operations: {set(dir(my_dtype)) - set(dir(object))}')
    def to_my_dtype(arr):
        return my_dtype(arr) if np.isscalar(arr) else np.vectorize(my_dtype)(arr)

    # Custom det_sum() function
    det_sum_cnt = 0
    det_sum_vals = 0
    def my_det_sum(products, odd_masks):
        """Extract values, run original det_sum(), then combine them back in my_dtype()"""
...
def test_det_of_columns():
    ...
    # Option 2:
    # Call det_of_columns() with single matrix, but feed mutliple via take_data()
