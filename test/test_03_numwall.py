...
def numwall_decomposition(data: np.array, degree: int, round_roots: int or None=None):
    ...
    np.testing.assert_allclose(determinant.det_sum_simple(minors * vals_arr[...,-minors.shape[-1]:], False), 0,
            atol=ABS_TOLERANCE)
    if not minors.any():
        print(f'No non-zero minor-determinants found')
        return None
    ...
    for roots, poly, vals, t in zip(roots_arr, minors, vals_arr, t_total):
        ...
        if roots.size:
            ...
            print(f'    Decomposed function:', ' + '.join(
