...
# TODO To be removed: Dummy `itertools.batched` for Python 3.10
if getattr(itertools, 'batched', None) is None:
    itertools.batched = lambda it, n: [tuple(it)]
...
# It makes possible processing of matrices larger 20x20, even if very slow.
# - 1000 - ~15-20 GiB with 25x25 matrix
# - 10000 - ~4-5 GiB with 20x20 matrix
# - 20000 - ~9-10 GiB with 20x20 matrix
...
def combinations_masks(size: int, comb_size: int, *, max_batch: int) -> Iterator[MaskArray]:
    ...
    if comb_size == size - 1:
        ...
        return
    for combs in itertools.batched(itertools.combinations(range(size), comb_size), max_batch):
        idxs = np.asarray(combs, dtype=IndexType)
